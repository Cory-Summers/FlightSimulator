...
namespace OpenGL
{
  ...
  template<typename T>
  GLuint CreateBuffer(std::vector<T> & buffer, uint64_t buffer_type = GL_ARRAY_BUFFER, uint64_t option = GL_STATIC_DRAW);
  template<typename T>
  GLuint CreateBuffer(std::vector<T> & buffer, uint64_t buffer_type, uint64_t option)
  {
    GLuint id;
    glGenBuffers(1, &id);
