...
void OpenGL::OrbitalPlane::CalcSegments()
{
  ...
  float rads = (2.0f * glm::pi<float>()) / static_cast<float>(m_segments);
  float theta = 0.f;
  float  scale = ellipse.semi_major / ellipse.semi_minor;
  circle_segment.resize(m_segments);
  for (size_t i = 0; i < m_segments; ++i)
  {
    circle_segment[i] = { ellipse.semi_minor * cosf(theta), 0.0f, ellipse.semi_minor * sinf(theta) };
    theta += rads;
  }
...
void OpenGL::OrbitalPlane::Draw(Camera & camera)
{
  ...
  glDrawArrays(GL_LINE_LOOP, 0, static_cast<GLsizei>(m_vertex.size()));
  glDisableVertexAttribArray(0);
}

OpenGL::OrbitalPlane::Ellipse::Ellipse()
  : semi_major()
  , semi_minor()
  , perigee()
  , apogee()
...
OpenGL::OrbitalPlane::Ellipse::Ellipse(Physics::StateElements const & elem)
  : semi_major(static_cast<float>(elem.semi_major) / (SIM_SCALE))
  , semi_minor(semi_major * std::sqrtf(1.f - static_cast<float>(elem.ecc)))
  , perigee((1.f - static_cast<float>(elem.ecc)) * semi_major)
  , apogee((1.f + static_cast<float>(elem.ecc)) * semi_major)
{
}
